...
export default function Hero() {
  return (
    <section id="home" className="relative overflow-hidden pt-24">
      ...
      <div className="relative mx-auto max-w-[90rem] px-3 py-24 md:px-5 md:py-32">
        <motion.h1
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="liquid-text floating-copy max-w-2xl text-4xl font-extrabold leading-[1.04] tracking-[-0.02em] md:text-6xl"
        >
          Luxury Hydration, Crafted For Modern Living.
        </motion.h1>
        <p className="mt-5 max-w-xl text-lg font-semibold text-slate-800 md:text-xl">
        ...
        <div className="mt-8 flex flex-wrap gap-4">
          <button
            ...
            className="rounded-full bg-gradient-to-r from-sky-600 to-blue-600 px-7 py-3.5 text-base font-bold text-white shadow-[0_12px_28px_rgba(2,132,199,0.38)] transition hover:-translate-y-0.5 hover:from-sky-500 hover:to-blue-500"
          >
            Shop Purifiers
          </button>
          <button
            onClick={() => document.getElementById("contact")?.scrollIntoView({ behavior: "smooth" })}
            className="rounded-full border border-slate-300 bg-white/85 px-7 py-3.5 text-base font-bold text-slate-900 backdrop-blur-md transition hover:-translate-y-0.5 hover:bg-white"
          >
            Book Free Demo
          </button>
        </div>
