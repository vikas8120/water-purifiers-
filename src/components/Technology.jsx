...
export default function Technology() {
  return (
    <section id="technology" className="mx-auto max-w-7xl px-4 py-20 md:px-8">
      <h2 className="text-4xl font-extrabold tracking-tight text-slate-900 md:text-5xl">Purification Technology</h2>
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-3 lg:grid-cols-6">
        {steps.map(({ icon: Icon, label }, idx) => (
          <motion.div
            ...
            className="group relative rounded-3xl border border-slate-200 bg-gradient-to-b from-white to-slate-50 p-5 text-center shadow-[0_8px_24px_rgba(15,23,42,0.06)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_14px_32px_rgba(14,116,144,0.16)]"
          >
            <div className="mx-auto inline-flex rounded-xl bg-sky-50 p-2.5 ring-1 ring-sky-100">
              <Icon className="text-sky-600" size={22} />
            </div>
            <p className="mt-3 text-[1.18rem] font-bold text-slate-900">{label}</p>
            {idx < steps.length - 1 && (
              <ArrowRight
                size={16}
